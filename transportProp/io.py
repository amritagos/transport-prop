...
__all__ = [ 'simple_write_xyz', 'my_read_lammps_data']

import numpy as np
from ase.atoms import Atoms # ASE stuff 
from ase.io import lammpsdata, read, write, lammpsrun
...
def simple_write_xyz(fileobj, image, comment=''):
    symbols = image.get_atomic_numbers()
    symbols1 = image.get_chemical_symbols()
    natoms = len(symbols1)
    fileobj.write('%d\n%s\n' % (natoms, comment))
    for s, (x, y, z) in zip(symbols, image.positions):

...
@njit
def msd_tau_t0(pos_t0, pos_t, dim, n_atoms):
...
@njit
def msd_tau(tau, n_origins, start_t0, positions, dim, n_atoms):
    ...
    r2_origins = np.zeros(n_origins)

    # Loop over all the time origins, starting from
    # t0 = start_t0
    for idx in range(n_origins):
        t0 = idx + start_t0  # current time origin
        t = t0 + tau  # time t = t0 + lag time
        # Positions at t0 and t
        pos_t0 = positions[t0]  # all x, y, z positions
        pos_t = positions[t]  # all x, y, z positions
        # Get slices for dim='x', 'y', 'z':
        pos_t0 = misc.get_slice(pos_t0, dim)
        pos_t = misc.get_slice(pos_t, dim)

        # Call the msd function for t0 and tau
        r2_origins[idx] = msd_tau_t0(pos_t0, pos_t, dim, n_atoms)

    # Get the average over all the permitted time origins
    return np.mean(r2_origins)
...
class MeanSquaredDisplacement:
    ...
    def __init__(
        self, traj, max_tau=None, start_t0=0, start_tau=1, delta_tau=1, dim="xyz"
    ):
        self.traj = traj  # Trajectory, list of Atoms objects
        self._get_position_array()  # get positions as a numpy ndarray

        self.n_frames = len(traj)  # Number of frames in the trajectory
        self.start_t0 = (
            start_t0  # The first time origin to calculate the MSD values from
        )
        ...
        if max_tau is None:
            ...
            self.max_tau = math.floor(
                0.5 * self.n_frames
            )  # to ensure averaging over the same number of time origins
        else:
        ...
        if dim in ["x", "y", "z", "xyz"]:
            ...
            print(
                "You have entered an invalid option for the dimension. Defaulting to xyz.\n"
            )
            self.dim = "xyz"

    def _get_position_array(self):
        # If the trajectory contains only a single step, ensure it is wrapped in a list.
        if not isinstance(self.traj, list):
            self.traj = [self.traj]

        # Create a NumPy ndarray of positions.
        # Each element is a 2D array of shape (n_atoms, 3), and the overall shape is (n_steps, n_atoms, 3).
        self.positions = np.array([atoms.get_positions() for atoms in self.traj])
